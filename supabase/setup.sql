...
--
-- Ordem: schema -> RLS -> funções e triggers -> documentos de saúde.
--
-- Depois disso:
...
/*
  Quantos documentos o paciente enviou hoje.

  Existe para conter abuso: sem teto, um upload em laço queimaria a cota da
  API de IA. SECURITY DEFINER porque a contagem precisa enxergar as linhas
  antes de a política de INSERT ser avaliada.
*/
create or replace function documents_today(target_patient uuid)
returns integer
language sql
stable
security definer
