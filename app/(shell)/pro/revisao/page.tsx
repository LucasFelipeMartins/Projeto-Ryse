...
import { BrainCircuit, ChevronRight, Clock, Inbox, ShieldCheck } from 'lucide-react';
import {
  Avatar,
  Badge,
  Card,
...
import { getReviewQueue } from '@/lib/queries/pro';
...
export default async function RevisaoPage() {
  ...
  const queue = await getReviewQueue(pro.id);

  return (
    <div className="space-y-5">
      <PageIntro
        eyebrow={queue.length > 0 ? `${queue.length} casos pendentes` : undefined}
        title="Revisão IA"
      ...
      {queue.length === 0 ? (
        <Card>
          <EmptyState
            ...
            description="Quando a IA gerar uma proposta a partir de check-ins ou exames, ela aparece aqui."
          />
        </Card>
      ) : (
        <>
              ...
              <Link key={c.id} href={`/pro/revisao/${c.id}`} className="tap block">
                <Card className="flex h-full flex-col transition-colors hover:border-line-strong">
                  ...
                  <div className="mt-4 border-t border-line pt-3.5">
                    ...
                    <Progress value={c.confidence} label={`Confiança da IA: ${c.confidence}%`} />
